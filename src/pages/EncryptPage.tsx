  ...
  async function handle(op: Op) {
    ...
    try {
      if (mode === "text") {
        const result =
          op === "encrypt"
            ? await invoke<string>("cmd_encrypt_text", { passphrase, plaintext: input })
            : await invoke<string>("cmd_decrypt_text", { passphrase, ciphertext: input });
        setOutput(result);
      } else {
        ...
        const result =
          op === "encrypt"
            ...
            : await invoke<string>("cmd_decrypt_file", { passphrase, filePath });
  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="space-y-3">
        ...
        {mode === "text" ? (
          <textarea placeholder="Paste text here…" value={input}
        ...
        <div className="flex gap-2">
          <button onClick={() => handle("encrypt")} disabled={loading}
            className="bg-blue-600 text-white px-5 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-50 transition-colors">
            Encrypt
          </button>
          <button onClick={() => handle("decrypt")} disabled={loading}
