...
export default function VaultPage() {
  ...
  async function load() {
    try { setCreds(await invoke<Credential[]>("list_credentials")); }
    ...
    <div className="max-w-3xl">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => { setForm(EMPTY); setEditId(null); setShowForm(true); }}
          className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700">+ Add</button>
      </div>
      ...
      {showForm && (
        <form onSubmit={save} className="bg-white border rounded-xl p-5 mb-4 space-y-3 shadow-sm">
          <h3 className="font-semibold text-sm">{editId ? "Edit" : "New credential"}</h3>
          {(["name", "username", "password", "url", "notes"] as const).map(f => (
          ...
          <div key={c.id} className="bg-white border rounded-xl px-5 py-4 flex justify-between items-start shadow-sm">
            <div>
              ...
              <div className="text-xs text-gray-400 mt-1 font-mono">
                ...
                <button onClick={() => setShowPw(p => ({ ...p, [c.id]: !p[c.id] }))}
                  className="ml-2 text-blue-500 hover:underline">{showPw[c.id] ? "hide" : "show"}</button>
