...
export default function QRPage() {
  ...
  async function generateQr() {
    try { setQrUrl(await invoke<string>("cmd_generate_qr", { content })); }
  ...
  async function decodeFile() {
    ...
    try { setDecoded(await invoke<string>("cmd_decode_qr_file", { filePath })); }
    catch (err) { setError(String(err)); }
  }
      ...
      {tab === "generate" && (
        <div className="space-y-4">
          <textarea value={content} onChange={e => setContent(e.target.value)}
            placeholder="Text or URL to encode…" rows={3}
        ...
        <div className="space-y-4">
          <div className="flex gap-2 flex-wrap">
            <label className="bg-gray-700 text-white px-4 py-2 rounded-lg text-sm font-medium cursor-pointer hover:bg-gray-800">
              Open image file
              <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={decodeFile} />
            </label>
